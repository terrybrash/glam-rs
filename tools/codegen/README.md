...
> **This directory is vendored.** It was a git submodule that pointed at
> [glam-codegen], and it is now plain files in this repository. The source
> commit is `673ed2c712d0c2db35fed00a21da7f132ab3cd7f`.
>
> This fork carries local changes to the tool and to its documentation, so a
> submodule that points at an upstream commit cannot hold them. A clone of
> this repository now needs no submodule step. To take a new version from
> upstream, copy the files and read the difference. See [ATTRIBUTION.md].
...
* `is_x86_64` - generate code using x86-64 intrinsics
* `is_aarch64` - generate code using ARM intrinsics

The backend names follow the target, not the instruction set level. The level
is set in one place only, `.cargo/config.toml`. See rewrite.md.
