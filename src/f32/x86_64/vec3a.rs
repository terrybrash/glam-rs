...
use crate::{f32::math, x86_64::*, BVec3, BVec3A, FloatExt, Quat, Vec2, Vec3, Vec4};
...
impl Vec3A {
    ...
    pub const USES_X86_64: bool = true;
    ...
    pub fn cross(self, rhs: Self) -> Self {
        unsafe {
            ...
            // FMA: `lhszxy_rhs - rhszxy * self`, with one rounding. The
            // subtracted product is the fused one. NEON below fuses the
            // same product, so the two backends keep the same bits.
            let sub = _mm_fnmadd_ps(rhszxy, self.0, lhszxy_rhs);
    ...
    pub fn abs(self) -> Self {
        Self(unsafe { crate::x86_64::m128_abs(self.0) })
    }
    ...
    pub fn is_finite_mask(self) -> BVec3A {
        BVec3A(unsafe { _mm_cmplt_ps(crate::x86_64::m128_abs(self.0), Self::INFINITY.0) })
    }
