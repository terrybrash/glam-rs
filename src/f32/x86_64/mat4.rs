...
impl Mat4 {
    ...
    pub fn project_point3a(&self, rhs: Vec3A) -> Vec3A {
        ...
        res = self.y_axis.mul_add(rhs.yyyy(), res);
        res = self.z_axis.mul_add(rhs.zzzz(), res);
        res = self.w_axis.add(res);
    ...
    pub fn mul_vec4(&self, rhs: Vec4) -> Vec4 {
        ...
        res = self.y_axis.mul_add(rhs.yyyy(), res);
        res = self.z_axis.mul_add(rhs.zzzz(), res);
        res = self.w_axis.mul_add(rhs.wwww(), res);
        res
    }
...
impl Mul for Mat4 {
    ...
    fn mul(self, rhs: Self) -> Self {
        unsafe {
            let x = m256_join(self.x_axis.0, self.x_axis.0);
            let y = m256_join(self.y_axis.0, self.y_axis.0);
            let z = m256_join(self.z_axis.0, self.z_axis.0);
            let w = m256_join(self.w_axis.0, self.w_axis.0);

            let mul_cols = |rhs: __m256| {
                let mut res = _mm256_mul_ps(x, _mm256_shuffle_ps::<0b00_00_00_00>(rhs, rhs));
                res = _mm256_fmadd_ps(y, _mm256_shuffle_ps::<0b01_01_01_01>(rhs, rhs), res);
                res = _mm256_fmadd_ps(z, _mm256_shuffle_ps::<0b10_10_10_10>(rhs, rhs), res);
                _mm256_fmadd_ps(w, _mm256_shuffle_ps::<0b11_11_11_11>(rhs, rhs), res)
            };

            let rhs = &rhs as *const Self as *const f32;
            let res01 = mul_cols(m256_load_cols(rhs));
            let res23 = mul_cols(m256_load_cols(rhs.add(8)));

            Self::from_cols(
                Vec4(m256_low(res01)),
                Vec4(m256_high(res01)),
                Vec4(m256_low(res23)),
                Vec4(m256_high(res23)),
            )
        }
    }
}
