...
impl Quat {
    ...
    pub fn mul_vec3a(self, rhs: Vec3A) -> Vec3A {
        unsafe {
            ...
            Vec3A(vaddq_f32(
                vaddq_f32(
                    // FMA: `w * w - b2`, with one rounding. This value is a
                    // scalar here, so `mul_add` gives the FMA, as rewrite.md
                    // requires. It matches `_mm_fmsub_ps` on x86-64 exactly.
                    vmulq_n_f32(rhs.0, w.mul_add(w, -b2)),
